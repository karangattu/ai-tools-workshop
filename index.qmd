...
::: {.hero-line}
The goal is not more tooling. The goal is a tight agent workflow: commit small, review fast, inspect the browser, fix with context intact.
:::

::: {.notes}
Set expectations: this is a hands-on workshop. Participants should have a terminal, a git repo, and an AI coding agent available.
:::

## Run Of Show

<div class="kicker"><i class="fa-solid fa-clock"></i> 90 minute workshop</div>

<div class="timeline">
<div class="timeline-row roborev"><strong>0-10 min</strong><p>Frame the agent workflow and install prerequisites.</p></div>
<div class="timeline-row roborev"><strong>10-45 min</strong><p>Section 1: RoboRev setup, review queue, fix paths, and daily usage.</p></div>
<div class="timeline-row"><strong>45-80 min</strong><p>Section 2: Agent Browser setup, refs, command patterns, and browser labs.</p></div>
<div class="timeline-row"><strong>80-90 min</strong><p>Capstone: combine both tools and prepare the deck for GitHub Pages.</p></div>
</div>

<div class="callout-strip"><i class="fa-solid fa-person-chalkboard"></i><p>Facilitator move: keep labs small. The value appears when participants repeat the loop several times, not when a single exercise becomes elaborate.</p></div>
...
<div class="badge-row"><span class="badge roborev">RoboRev = code review memory</span><span class="badge browser">Agent Browser = page interaction memory</span><span class="badge">Small commits make both better</span></div>
...
- Keeps hooks compatible with hook managers such as Husky.
:::
::::

## Lab 1: First Review Loop
...
Create a small validation helper and a focused unit test. Keep the change intentionally small so RoboRev can review one idea at a time.
:::
::::

::: {.notes}
If participants do not have a suitable repo, have them create a tiny scratch repo with a single function and test. The point is the review loop, not the app domain.
:::
...
<div class="callout-strip"><i class="fa-solid fa-keyboard"></i><p>In the TUI, use arrow keys or `j` and `k` to move, `Enter` to open details, `y` to copy a completed review, and `?` for shortcuts.</p></div>
...
Run `roborev skills install`, then use `/roborev-fix` in Claude Code or `$roborev-fix` in Codex.
:::
...
<div class="callout-strip"><i class="fa-solid fa-bullseye"></i><p>Workshop checkpoint: participants should be able to explain where a review lives, how to inspect it, and how to resolve it without losing the original coding context.</p></div>
...
roborev summary --since 7d
roborev insights --since 30d
```
:::
::::
...
- Add local review guidelines for recurring project risks.
- Use `--min-severity` when auto-fixing noisy queues.
- Use `roborev compact` when many open findings need consolidation.
:::
::::

## RoboRev Troubleshooting Drill

| Symptom | First check | Useful command |
|---|---|---|
| Reviews are not appearing | Did a commit happen after init? | `roborev status` |
| Hook looks wrong | Is `.git/hooks/post-commit` present? | `roborev install-hook --force` |
| Agent is unavailable | Are agent CLIs and API keys configured? | `roborev check-agents` |
| Review is stuck | Did the daemon or job fail? | `roborev log <job_id>` |
| Queue is noisy | Are low severity findings useful? | `roborev fix --min-severity medium` |
...
roborev config list --global --show-origin

# Check agent is healthy
roborev check-agents --agent claude-code
