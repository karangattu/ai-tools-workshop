...
```bash
agent-browser open https://posit.co/
```
...
```text
- heading "This website uses cookies" [level=2, ref=e139]
- link "Show details" [ref=e38]
- button "OK" [ref=e39]
...
<div class="callout-strip"><i class="fa-solid fa-terminal"></i><p>Try without installing: `npx agent-browser open posit.co`.</p></div>
:::
::::
...
agent-browser open https://posit.co/
agent-browser snapshot -i
agent-browser click @e50
agent-browser screenshot /Users/name/Desktop/result.png
agent-browser close
```
:::
::::
...
agent-browser type "Buy Dr. Pepper"
...
## Prompting Claude Code For Browser Tasks (cont.)
:::: {.grid-2}
::: {.card}
...
## Act 4: Exploratory Browser Task
...
Use Agent Browser to test https://demo.playwright.dev/todomvc/ with this todo text:
...
<div class="kicker"><i class="fa-solid fa-compare"></i> Comparison</div>

### Example: "Log in to a site"

| Approach | How it works | Agent perspective |
|---|---|---|
| **Playwright CLI** | Find selector `input[name="user"]`, wait for visibility, type text, handle timing | Requires CSS knowledge, brittle selectors, more "coding" |
| **Agent Browser** | `snapshot -i` → see `@e1` (username), `@e2` (password) → `fill @e1 "user"` → `fill @e2 "pass"` → `click @e3` | Natural, human-like, deterministic refs, no CSS needed |

## Choosing The Browser Tool

<div class="command-grid">
<div class="command-card"><i class="fa-solid fa-bolt"></i> **One-off task**<span>Agent Browser — move fast with minimal setup</span></div>
<div class="command-card"><i class="fa-solid fa-magnifying-glass"></i> **Deep reasoning over live state**<span>Playwright MCP — introspect and reason in real-time</span></div>
<div class="command-card"><i class="fa-solid fa-repeat"></i> **Repeatable CI suite**<span>Playwright Test — durable, multi-browser, version-controlled</span></div>
<div class="command-card"><i class="fa-solid fa-coins"></i> **Token constrained**<span>Agent Browser — compact snapshots (~200-400 tokens vs 3000+)</span></div>
</div>
